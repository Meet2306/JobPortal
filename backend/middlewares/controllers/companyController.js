...
exports.updateProfile = async (req, res) => {
    try {
        ...
        if (req.body.hrContactNumber && !phoneRegex.test(req.body.hrContactNumber)) {
            return res.status(400).json({ error: 'HR Phone must be exactly 10 digits and start with 7, 8, or 9' });
        }
        if (req.body.phoneNumber && !phoneRegex.test(req.body.phoneNumber)) {
            return res.status(400).json({ error: 'Company Phone number must be exactly 10 digits and start with 7, 8, or 9' });
        }
